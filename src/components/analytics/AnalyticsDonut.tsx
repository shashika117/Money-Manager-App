...
export function AnalyticsDonut({
  tab, bounds, view, focus, activeMonth, canDrillBack, onSelectBucket, onDrillBack,
}: Props) {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={wrapRef} className="relative mx-auto" style={{ width: SIZE, height: SIZE }}
            ...
            <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="w-full h-full animate-fade-in-scale">
              {arcs.map(a => {
                ...
                const common = {
                  ...
                  onMouseMove:  moveTip,
                  onClick:      () => onSelectBucket(a.bucket),
                }
          ...
          {/* ── Legend (clickable; non-zero rows only) ── */}
