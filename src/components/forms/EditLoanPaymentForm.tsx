...
import { useUpdateLoanPayment } from '@/hooks/useLoanPaymentMutations'
import type { LoanPaymentGroupData } from '@/hooks/useEditTransaction'

interface EditLoanPaymentFormProps {
  data:       LoanPaymentGroupData
  onSuccess:  () => void
  onCancel:   () => void
}
...
const schema = z
  ...
  .superRefine((d, ctx) => {
    if (d.from_account && d.loan_account && d.from_account === d.loan_account) {
      ctx.addIssue({
...
export function EditLoanPaymentForm({ data, onSuccess, onCancel }: EditLoanPaymentFormProps) {
  const { data: accounts = [], isLoading: accLoading } = useAccounts()
  const updateLoanPayment = useUpdateLoanPayment()
  ...
  const {
    ...
    defaultValues: {
      ...
      capital_amount:  String(data.capital_amount),
      interest_amount: data.interest_amount > 0 ? String(data.interest_amount) : '',
      note:            data.note ?? '',
    },
  })

  const watchFromAccount = watch('from_account')
  const watchCapital      = parseFloat(watch('capital_amount'))  || 0
  const watchInterest     = parseFloat(watch('interest_amount') || '0') || 0
  const totalEmi          = watchCapital + watchInterest

  async function onSubmit(form: FormData) {
    try {
      await updateLoanPayment.mutateAsync({
        transfer_group_id: data.transfer_group_id,
        date:               form.date,
        from_account:       form.from_account,
        loan_account:       form.loan_account,
        capital_amount:     parseFloat(form.capital_amount),
        interest_amount:    form.interest_amount ? parseFloat(form.interest_amount) : 0,
        note:               form.note ?? '',
      })
      onSuccess()
    } catch (err) {
      console.error('Loan payment update failed:', err)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-4 pb-2">
      ...
      <div>
        ...
        />
        {errors.date && <p className="mt-1 font-dm text-xs text-red">{errors.date.message}</p>}
      </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          >
            {accounts.map(acc => (
              <option key={acc.id} value={acc.master_account}>{acc.master_account}</option>
            ))}
          </select>
      ...
      <div>
        <label className="mb-1.5 block font-dm text-xs font-medium uppercase tracking-wider text-soft">Loan Account</label>
        <div className="relative">
          <select
        ...
        {errors.loan_account && <p className="mt-1 font-dm text-xs text-red">{errors.loan_account.message}</p>}
      </div>

      <div>
        <label className="mb-1.5 block font-dm text-xs font-medium uppercase tracking-wider text-soft">Capital Amount (Rs)</label>
        <input
        ...
        />
        {errors.capital_amount && <p className="mt-1 font-dm text-xs text-red">{errors.capital_amount.message}</p>}
      </div>

      <div>
        <label className="mb-1.5 block font-dm text-xs font-medium uppercase tracking-wider text-soft">
          Interest Amount (Rs) <span className="normal-case text-muted">(optional)</span>
        </label>
        <input
          type="number" inputMode="decimal" step="0.01" min="0" placeholder="0.00"
          {...register('interest_amount')}
          className="w-full rounded-xl border border-line bg-panel px-4 py-3 font-dm text-sm text-white placeholder:text-muted outline-none transition-colors focus:border-amber"
        />
        {errors.interest_amount && <p className="mt-1 font-dm text-xs text-red">{errors.interest_amount.message}</p>}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          {...register('note')}
          className="note-field w-full rounded-xl border border-line bg-panel px-4 py-3 font-dm text-sm text-white placeholder:text-muted outline-none transition-colors focus:border-amber"
        />
      </div>

      {updateLoanPayment.isError && (
        <div className="rounded-xl border border-red/30 bg-red/10 px-4 py-3">
          <p className="font-dm text-sm text-red">
            {updateLoanPayment.error?.message ?? 'Update failed. Try again.'}
          </p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={updateLoanPayment.isPending}
        className={cn(
          'mt-2 w-full rounded-xl bg-amber py-4 font-sora text-sm font-semibold text-white transition-all active:scale-[0.98]',
          updateLoanPayment.isPending ? 'opacity-60 cursor-not-allowed' : 'hover:opacity-90',
        )}
      >
        {updateLoanPayment.isPending ? 'Saving…' : 'Save Changes'}
      </button>
